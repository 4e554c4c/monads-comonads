...
record R : Set where
  field
    ...
    b : B
\end{minted}
is the identification of an element \mintinline{agda}{x : R} with any record formed from its projections \mintinline{agda}{record { a = R.a x ; b = R.b x }}. This is necessary in Agda-categories in order to judgementally identify the dual dual category ${C^{\op}}^{\op}$ with $C$, as the two are not syntactically equal but rather evaluate to the same result along each projection.
